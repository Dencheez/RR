...
const ContactModal = ({ isOpen, onClose, isDark, t }) => {
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className={`relative w-full max-w-md p-8 rounded-[40px] border shadow-2xl ${isDark ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-black/5'}`}
        >
          <button
            onClick={onClose}
          ...
          <div className="text-center mb-8">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-[#c9a227] to-[#e5c158] mx-auto mb-4 flex items-center justify-center shadow-xl shadow-[#c9a227]/20">
              <Phone size={32} className="text-white" />
            </div>
            <h3 className={`text-2xl font-bold mb-2 ${isDark ? 'text-white' : 'text-[#1a1a1a]'}`}>
              {t('Contact Agent') || 'Contact Agent'}
            </h3>
            <p className={`text-sm opacity-50 ${isDark ? 'text-white' : 'text-black'}`}>
          ...
          <div className="space-y-4">
            <a
              ...
              className={`flex items-center gap-4 p-5 rounded-3xl border transition-all hover:scale-[1.02] active:scale-[0.98] ${isDark ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-black/5 border-black/10 hover:bg-black/10'}`}
            >
              <div className="p-3 bg-[#c9a227] rounded-2xl text-white">
                <Phone size={20} />
              </div>
              <div className="text-left">
                <p className={`text-[10px] font-black uppercase tracking-widest opacity-40 ${isDark ? 'text-white' : 'text-black'}`}>Call Now</p>
                <p className={`text-lg font-bold ${isDark ? 'text-white' : 'text-black'}`}>+966 50 000 0000</p>
              </div>
            </a>
            ...
            >
              ...
              <div className="text-left">
                <p className={`text-[10px] font-black uppercase tracking-widest opacity-40 ${isDark ? 'text-white' : 'text-black'}`}>WhatsApp</p>
                <p className={`text-lg font-bold ${isDark ? 'text-white' : 'text-black'}`}>Send Message</p>
              </div>
            </a>
          </div>
        </motion.div>
...
const PropertyDetail = () => {
  ...
  const [actionType, setActionType] = useState('buy'); // 'buy' or 'rent'
  ...
  // Theme classes
  const pageBg = isDark ? "bg-[#0d0d0d] text-white" : "bg-[#f7f5f0] text-[#1a1a1a]";
  const cardBg = isDark ? "bg-[#1a1a1a] border-white/5" : "bg-white border-black/8 shadow-md";
  const backBtn = isDark
    ? "bg-[#1a1a1a]/60 border-white/10 text-white hover:bg-[#c9a227] hover:border-[#c9a227]"
    : "bg-white/90 border-black/10 text-[#1a1a1a] hover:bg-[#c9a227] hover:text-white hover:border-[#c9a227]";
  ...
  const amenityCard = isDark ? "bg-white/5 border-white/5 hover:border-[#c9a227]/30" : "bg-black/5 border-black/5 hover:border-[#c9a227]/40";
  const amenityText = isDark ? "text-gray-500 group-hover:text-white" : "text-gray-500 group-hover:text-[#1a1a1a]";
  const locationBox = isDark ? "bg-white/5 border-white/10" : "bg-black/5 border-black/10";
  const specBox = isDark ? "bg-white/5 rounded-2xl" : "bg-black/5 rounded-2xl";
  const outlineBtn = isDark
    ? "border-white/10 text-white hover:bg-white/5"
    : "border-black/10 text-[#1a1a1a] hover:bg-black/5";
  const saveBtn = isDark
    ? "bg-white/5 border-white/10 text-white/30 hover:border-[#c9a227]"
    : "bg-black/5 border-black/10 text-black/30 hover:border-[#c9a227]";

  return (
    <div className={`min-h-screen font-sans overflow-x-hidden transition-colors duration-300 ${pageBg}`}>
      {/* Floating iOS-style Back Button */}

      <motion.button
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        onClick={() => navigate(-1)}
        className={`fixed top-8 left-8 z-[100] flex items-center gap-2 backdrop-blur-xl border px-5 py-3 rounded-full transition-all duration-500 group shadow-2xl ${backBtn}`}
      >
        <span className="text-xl group-hover:-translate-x-1 mb-1 transition-transform">←</span>
      ...
      <main className="max-w-[1440px] mx-auto px-4 md:px-10 pt-24 md:pt-20 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="col-span-1 lg:col-span-8 order-1">
            <div className={`relative rounded-[40px] overflow-hidden border shadow-2xl ${cardBg}`}>
              <Swiper
              ...
              <div className="absolute top-8 left-8 z-10 bg-[#c9a227] text-white text-xs font-black px-6 py-2.5 rounded-full uppercase tracking-widest shadow-lg">
                {propertyData.status}
              </div>
            </div>
          </div>

          {/* Right: Info & Contact Sidebar */}
          <div className="col-span-1 lg:col-span-4 order-2 lg:row-span-3">
            <div className="lg:sticky lg:top-32 space-y-8">
              {/* Main Info Card */}
              <div className={`p-6 md:p-10 rounded-[40px] border shadow-2xl ${cardBg}`}>
                <div className="flex justify-between items-start mb-6">
                  <h1 className={`text-3xl md:text-4xl font-bold tracking-tight ${titleColor}`}>{propertyData.title}</h1>
                  <button
                    onClick={() => setIsSaved(!isSaved)}
                    className={`p-4 rounded-2xl border transition-all duration-300 ${isSaved ? 'bg-[#c9a227] border-[#c9a227] text-white' : saveBtn}`}
                  >
                ...
                <div className="space-y-4">
                  {propertyData.type === "For Sale" ? (
                    <button
                      ...
                      className="w-full py-4 md:py-5 bg-[#c9a227] text-white rounded-2xl font-black uppercase tracking-[0.15em] text-xs shadow-xl shadow-[#c9a227]/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3"
                    >
                      {t('Buy')}
                    </button>
                  ) : (
                    <button
                      ...
                      className={`w-full py-4 md:py-5 rounded-2xl font-black uppercase tracking-[0.15em] text-xs hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 ${isDark ? "bg-white text-black" : "bg-[#1a1a1a] text-white"}`}
                    >
                      {t('Rent')}
                    </button>
                  )}
                  <div className="flex gap-4 pt-2">
                    <button className={`flex-1 py-3 md:py-4 border rounded-2xl font-black uppercase tracking-[0.1em] text-[10px] transition-all ${outlineBtn}`}
              ...
              <div className="bg-gradient-to-br from-[#c9a227]/20 to-transparent p-6 md:p-8 rounded-[40px] border border-[#c9a227]/20 flex items-center gap-4 md:gap-6">
                <div className="p-3 md:p-4 bg-[#c9a227] rounded-2xl text-white shadow-lg">
          ...
          {/* Amenities Section */}
          <div className={`col-span-1 lg:col-span-8 order-3 p-6 md:p-12 rounded-[40px] border shadow-inner ${cardBg}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
              {[
                ...
                <div key={idx} className={`flex flex-col items-center gap-4 p-4 md:p-6 rounded-3xl border transition-all group ${amenityCard}`}>
                  <span className="group-hover:scale-110 transition-transform">{item.icon}</span>
                  <span className={`text-[10px] font-black uppercase tracking-[0.2em] transition-colors text-center ${amenityText}`}>{item.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Location Map */}
          <div className={`col-span-1 lg:col-span-8 order-4 p-6 md:p-12 rounded-[40px] border overflow-hidden shadow-inner ${cardBg}`}>
            ...
            <div className={`relative h-[300px] md:h-[400px] rounded-[32px] md:rounded-[40px] overflow-hidden border shadow-inner ${cardBg} cursor-grab active:cursor-grabbing`}>
              <motion.div
                drag
              ...
              >
                <div className="w-full h-full relative">
                  <iframe
                    ...
                    className={`w-full h-full border-none pointer-events-none ${isDark ? 'grayscale invert-[0.94] contrast-[1.1] brightness-[0.9] opacity-70' : 'grayscale opacity-80'}`}
                    allowFullScreen=""
                    loading="lazy"
                  />
                  <div className={`absolute inset-0 bg-gradient-to-t ${isDark ? "from-[#1a1a1a]/40" : "from-white/40"} via-transparent to-transparent pointer-events-none`} />

                  {/* Centered Property Marker */}
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
                    <div className="relative">
                      <div className="bg-[#c9a227] text-white px-6 py-3 rounded-2xl text-xs font-black shadow-2xl whitespace-nowrap mb-4 relative z-10">
                        {propertyData.price}
                        <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-[#c9a227] rotate-45" />
                      </div>
                      <div className="w-8 h-8 rounded-full border-4 border-[#c9a227] bg-white flex items-center justify-center mx-auto shadow-xl">
